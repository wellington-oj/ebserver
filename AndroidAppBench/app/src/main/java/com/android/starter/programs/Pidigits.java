...
public class Pidigits {
    ...
    public void runCode(int n, FileOutputStream writer) throws IOException {
        ...
        while (n > 0){
            if (n >= L){
                for (int i=0; i<L; i++) writer.write((""+digits.next()).getBytes());
                j += L;
            } else {
                for (int i=0; i<n; i++) writer.write((""+digits.next()+"\n").getBytes());
                for (int i=n; i<L; i++) writer.write(" \n".getBytes());
                j += n;
            }

            writer.write("\t:".getBytes()); writer.write((""+j+"\n").getBytes());
            n -= L;
        }
    }
}
